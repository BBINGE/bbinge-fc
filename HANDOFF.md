...
- about 페이지 — 흰 계열 개인 공식 사이트형 에디토리얼로 개편(2026-08-12). 1200px 비대칭 히어로, 컬러 프로필, `@삥이` 워터마크, SKILL 태그, 이력 2트랙, 공식 로고형 온라인 권위 카드, 주요 의뢰인·성과 패널, 네이버·인스타그램·이메일 CTA를 적용. 로고 원본 12개는 `public/images/about-logos/`에 영문 파일명으로 보관하고 48px 원형 슬롯 안에서 비율을 유지한다. 브런치스토리·티스토리는 겹친 2개 슬롯으로 표시. 확정 문구와 이력·성과 본문은 유지. 로컬 380/768/1440px에서 가로 넘침 없음, Pretendard 및 스크롤 순차 등장 확인.
