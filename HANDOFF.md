...
- 일반 글 컬렉션 `src/content/articles/`에는 4개 파일이 있다. `why-eredivisie-became-selling-league.md`는 완성·공개된 첫 축세 장문 칼럼이다. `supurga-pilgrimage.md`는 공개 상태지만 본문 자리표시자이며, `3141.md`와 `423423423.md`는 `draft: true`인 발행 흐름 시험 산출물이다. 수페르가 글과 시험 파일을 실제 완성 글로 오인하지 않는다.
...
- 현재 RSS에는 공개된 1916·1917 코파 아메리카 아카이브, 에레디비시 축세 칼럼, 공개 상태인 수페르가 자리표시자 글이 들어간다. 수페르가를 완성하거나 공개 제외하기 전까지 이를 실제 완성 글로 소개하지 않는다.
