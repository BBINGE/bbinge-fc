...
- **[최우선] 블로그 에디터 시스템** — `EDITOR_SPEC.md` v1.1 확정(2026-07-28, 착수 결정 포함). 사이트의 존재 이유는 운영자만 쓸 수 있는 글이며, 게임·GOAT는 애드센스 유입 기반일 뿐. 스마트에디터 원 수준 위지윅(오픈소스 기반)+R2 이미지 업로드+유튜브 임베드+카테고리 데이터화+자동저장·예약발행. **확정된 착수 결정**: 글은 정적 마크다운 파일로 GitHub 커밋(운영자 자산화·자생 원칙), 로그인은 기존 GitHub OAuth 재사용, 초안은 로컬 자동저장 후 발행 시에만 커밋. 구현은 §6의 6단계를 나눠서 진행. 이 프로젝트의 메인 과제
