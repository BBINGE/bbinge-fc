...
- 축세 칼럼의 이미지 캡션은 단순한 사진명이 아니라 운영자의 서사와 해석을 이어가는 본문의 일부로 취급한다. 임의로 축약하지 않으며 본문보다 작은 기존 13px 계층은 유지한다. 반대로 글 맨 아래 `자료와 확인 경로`는 독자의 흐름을 방해하지 않는 각주 계층(데스크톱 12px·모바일 11px)으로 낮춘다. 클럽·국가대표 성과표는 가능한 범위에서 각 조직의 공식 색감을 반영한다.
