...
- about 페이지 — 흰 계열 개인 공식 사이트형 에디토리얼로 개편(2026-08-12). 1200px 비대칭 히어로, 컬러 프로필, `@삥이` 워터마크, SKILL 태그, 이력 2트랙, 공식 로고형 온라인 권위 카드, 주요 의뢰인·성과 패널, 네이버·인스타그램·이메일 CTA를 적용. 히어로와 이력 사이에는 편지형 `나는 왜 축구를 쓰는가` 섹션을 추가(2026-08-13)했으며, 원고는 운영자가 직접 작성할 때 `footballLetter` 배열에 문단 단위로 넣는다. 현재는 의도적인 준비 안내 문구가 노출된다. 로고 자산은 `public/images/about-logos/`에 영문 파일명으로 보관하고 48px 원형 슬롯 안에서 비율을 유지한다. 온라인 권위 문구는 `플랫폼 + 활동/자격 + (계정명·보충정보)` 형식을 사용하며 복수 계정은 가운데점으로 구분한다. NAVER 인물검색과 인플루언서 공인은 별도 카드이며, 브런치 작가(@삥이)와 티스토리 저자(@BBingStory)도 서로 다른 삥이 계열 브랜드로 별도 표시한다. 확정 이력·성과 본문은 유지. 로컬 380/768/1440px에서 가로 넘침 없음, Pretendard 및 스크롤 순차 등장 확인.
