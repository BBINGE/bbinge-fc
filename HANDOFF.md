...
- 일반 칼럼과 아카이브 상세의 공통 공유 영역은 `ShareButtons.astro`를 사용한다. 제목은 `이 글이 좋았다면 공유해 주세요!`이며 링크복사·카카오·X·페이스북에 각각 SVG 아이콘을 표시한다. 480px 이하에서는 2열 버튼으로 전환한다. 카카오 JavaScript 키가 설정되기 전에는 기존처럼 링크 복사로 대체한다.
