...
이 사이트의 존재 이유는 운영자(삥이)만 쓸 수 있는 글이다: 축구 지식 × 세계사·철학·인문학 × 문장력 × 대체불가 주제 × 디자인·가독성 집요함. 게임(GOAT)·아카이브·디자인은 애드센스 유입을 위한 기반이며 목적이 아니다. 따라서 블로그 에디터 시스템(`EDITOR_SPEC.md`)이 최우선 과제이고, 그 유일한 성공 기준은 "운영자가 장문 글쓰기에 피로를 느끼지 않는 것"이다. 어떤 개발 판단도 이 우선순위를 뒤집지 않는다.
