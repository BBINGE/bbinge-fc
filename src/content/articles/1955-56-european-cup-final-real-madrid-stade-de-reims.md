---
title: '1955-56 유러피언컵 결승전 H/L: 레알 마드리드 CF vs 스타드 드 랭스'
description: '두 번 앞선 스타드 드 랭스와 두 번 따라붙은 레알 마드리드 CF. 1956년 파리에서 열린 첫 유러피언컵 결승을 열두 개의 기록 영상으로 다시 본다.'
category: 'european-cup'
tags: ['1955-56 유러피언컵', '레알 마드리드 CF', '스타드 드 랭스', '유러피언컵 결승전', '알프레도 디스테파노', '엑토르 리알', '축구 하이라이트']
pubDate: 2026-09-10T21:30:00+09:00
coverImage: '/images/highlights/1955-56-european-cup-final-cover.jpg'
cardImage: '/images/highlights/1955-56-european-cup-final-kopa-di-stefano.jpg'
cardImageAlt: '스타드 드 랭스의 레몽 코파와 레알 마드리드 CF의 알프레도 디스테파노가 악수하는 결승전 썸네일'
coverImageAlt: '1955-56 유러피언컵 결승전을 앞두고 잔디 위에 놓인 유러피언컵 트로피'
coverImageCaption: '1956년 6월 13일, 파리에서 유럽 챔피언 클럽 대회의 첫 결승전이 열렸다.'
coverImageWidth: 960
coverImageHeight: 470
priorPublication:
  platform: 'NAVER'
  url: 'https://blog.naver.com/sho3603/223467845654'
  relation: 'revised-edition'
draft: false
featured: false
relatedGroup: 'european-cup-1955-56'
---

<div class="european-cup-title-card" aria-label="1955-56 유러피언컵 결승전 부제">
  <span>COUPE DES CLUBS CHAMPIONS EUROPÉENS · FINAL</span>
  <strong lang="es">LA PRIMERA</strong>
  <small>라 프리메라; 스페인어로 ‘첫 번째’라는 뜻. 첫 유러피언컵 결승을 여는 이름.</small>
</div>

<div class="match-board european-cup-match-board" aria-label="1955-56 유러피언컵 결승전 대진">
  <p>1955-56 EUROPEAN CUP · FINAL</p>
  <div class="match-board-teams">
    <div><img class="match-board-crest" src="/images/archive/teams/clubs/real-madrid-1941.svg" alt="레알 마드리드 CF 문장" width="80" height="88" /><strong>레알 마드리드 CF</strong><small>REAL MADRID CF</small><span class="match-board-country"><img src="/images/flags/es-1945.png" alt="1945년부터 사용된 스페인 국기" width="28" height="18" /><span>스페인</span></span></div>
    <span><b class="match-board-vs">V</b><em>90 MINUTES<br />UNPLAYED</em><b class="match-board-vs">S</b></span>
    <div><img class="match-board-crest" src="/images/archive/teams/clubs/reims-1931.svg" alt="스타드 드 랭스 문장" width="80" height="88" /><strong>스타드 드 랭스</strong><small>STADE DE REIMS</small><span class="match-board-country"><img src="/images/flags/fr.svg" alt="프랑스 국기" width="28" height="18" /><span>프랑스</span></span></div>
  </div>
  <dl><div><dt>날짜</dt><dd>1956년 6월 13일</dd></div><div><dt>장소</dt><dd>파르크 데 프랭스</dd></div><div><dt>관중</dt><dd>38,239명</dd></div><div><dt>주심</dt><dd>아서 엘리스</dd></div></dl>
</div>

우리는 이 결승전을 너무 늦게 알고 있다. 레알 마드리드가 유러피언컵을 다섯 번 연속 우승했다는 결말을 이미 배운 다음 첫 번째 우승으로 되돌아가기 때문이다. 그러면 1956년 6월 13일의 스타드 드 랭스는 왕조의 첫 계단처럼 보이고, 레알의 승리는 아직 일어나지도 않은 네 번의 우승에 의해 미리 보증된 사건처럼 읽힌다.

당시 선수들에게는 그런 특권이 없었다. 파코 헨토<span class="foreign-note" lang="es">(Francisco Gento López; 스페인 왼쪽 윙어, 1933-2022)</span>는 초대 대회를 회고하며 외국 팀의 전술을 알아볼 텔레비전이나 영상이 없었고, 상대를 거의 모른 채 경기에 들어갔다고 말했다.<a class="cite" href="#source-2" aria-label="출처 2">[2]</a> 어느 팀도 유러피언컵 결승을 치러본 적이 없었고, 어느 주장도 이 트로피를 들어본 적이 없었다. 그날의 ‘최초’는 두 팀 앞에 똑같이 놓인 빈칸이었다.

그 빈칸을 먼저 자기 쪽으로 끌어당긴 팀은 랭스였다.

## 전반 6분: 폭을 벌린 뒤, 르블롱이 안으로 들어오다

<section class="highlight-scene european-cup-scene">
  <header><span>6' · STADE DE REIMS 1-0</span><h3>미셸 르블롱, 첫 결승의 첫 골</h3></header>
  <div class="highlight-clip-grid two-clips">
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="미셸 르블롱의 1956 유러피언컵 결승전 선제골"><source src="/media/highlights/1955-56-european-cup-final/01-leblond-6.mp4" type="video/mp4" /></video><figcaption>르블롱이 수비 사이를 통과해 낮은 슈팅으로 첫 골을 넣었다.</figcaption></figure>
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="미셸 이달고가 레알 마드리드 수비를 상대로 측면에서 드리블하는 장면"><source src="/media/highlights/1955-56-european-cup-final/02-hidalgo-wing.mp4" type="video/mp4" /></video><figcaption>이달고는 오른쪽에서 공을 멈추고 방향을 바꾸며 레알의 측면 수비를 계속 흔들었다.</figcaption></figure>
  </div>
  <p>알베르 바토<span class="foreign-note" lang="fr">(Albert Batteux; 프랑스 감독, 1919-2003)</span>의 계획은 레알을 정면에서 밀어내는 데 있지 않았다. 스타드 드 랭스의 70주년 회고에 따르면 양쪽의 미셸 이달고와 장 탕플랭이 속도로 측면을 돌아 들어가고, 중앙의 레몽 코파가 움직일 공간을 만드는 구상이었다.<a class="cite" href="#source-6" aria-label="출처 6">[6]</a> 기록 영상에서도 이달고가 터치라인에 붙어 직선으로만 달리는 대신 공을 세우고 방향을 바꾸며 수비수를 바깥으로 끌어내는 장면이 먼저 보인다.</p>
  <p>그 틈을 하프백 미셸 르블롱<span class="foreign-note" lang="fr">(Michel Leblond; 프랑스 하프백, 1932-2009)</span>이 사용했다. 전반 6분, 뒤에서 출발한 르블롱이 수비 사이로 들어가 낮은 슈팅을 밀어 넣었다. 첫 유러피언컵 결승의 첫 득점자는 코파도, 알프레도 디스테파노도 아니었다. 폭을 만든 공격수들 뒤에서 안쪽 길을 읽은 미드필더였다.</p>
</section>

## 전반 10분: 골키퍼가 나온 뒤에도 끝나지 않은 공격

<section class="highlight-scene european-cup-scene">
  <header><span>10' · STADE DE REIMS 2-0</span><h3>장 탕플랭, 4분 만에 두 골 차</h3></header>
  <div class="highlight-clip-grid two-clips">
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="장 탕플랭의 1956 유러피언컵 결승전 두 번째 골"><source src="/media/highlights/1955-56-european-cup-final/03-templin-10.mp4" type="video/mp4" /></video><figcaption>알론소가 전진해 몸을 던졌지만 공은 멈추지 않았고, 탕플랭이 남은 공을 골문 안으로 보냈다.</figcaption></figure>
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="스타드 드 랭스의 두 번째 골에 환호하는 파르크 데 프랭스 관중"><source src="/media/highlights/1955-56-european-cup-final/04-reims-crowd.mp4" type="video/mp4" /></video><figcaption>10분 만에 2-0. 파리의 관중석이 프랑스 챔피언의 두 번째 골에 반응한다.</figcaption></figure>
  </div>
  <p>이번에는 왼쪽이었다. 장 탕플랭<span class="foreign-note" lang="fr">(Jean Templin; 프랑스 측면 공격수, 1928-1980)</span>이 뒷공간으로 달렸고, 후안 알론소<span class="foreign-note" lang="es">(Juan Adelarpe Alonso; 스페인 골키퍼, 1927-1994)</span>가 골문을 비우고 전진했다. 영상만으로 접촉의 책임을 한 사람에게 단정할 수는 없다. 분명한 것은 알론소가 몸을 던진 뒤에도 공이 살아 있었고, 탕플랭이 균형을 잃지 않은 채 빈 골문으로 마무리했다는 사실이다.</p>
  <p>전반 10분, 2-0. 레알은 첫 실점에 답하기도 전에 두 번째 골을 내줬다. 수비 뒤로 공이 넘어가자 알론소는 골문에 남아 슈팅을 기다리는 대신 앞으로 나왔지만, 그 전진으로도 공격을 끝내지 못했다. 골키퍼가 비운 자리를 탕플랭이 먼저 이용했다. 랭스의 속도는 수비수를 뒤로 달리게 하는 데서 그치지 않고 골키퍼에게도 결정을 재촉하고 있었다.</p>
</section>

## 전반 14분과 30분: 두 골을 되찾은 16분

<section class="highlight-scene european-cup-scene">
  <header><span>14' · 1-2 / 30' · 2-2</span><h3>디스테파노가 줄이고, 리알이 원점으로 돌리다</h3></header>
  <div class="highlight-clip-grid two-clips">
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="알프레도 디스테파노의 1956 유러피언컵 결승전 만회골"><source src="/media/highlights/1955-56-european-cup-final/05-di-stefano-14.mp4" type="video/mp4" /></video><figcaption>디스테파노가 골키퍼보다 먼저 낮게 마무리해 두 골 차를 4분 만에 줄였다.</figcaption></figure>
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="엑토르 리알의 1956 유러피언컵 결승전 동점골"><source src="/media/highlights/1955-56-european-cup-final/06-rial-30.mp4" type="video/mp4" /></video><figcaption>문전으로 이어진 공격을 리알이 마무리하면서 결승은 30분 만에 다시 시작점으로 돌아왔다.</figcaption></figure>
  </div>
  <p>두 번째 실점에서 4분이 지난 14분, 알프레도 디스테파노<span class="foreign-note" lang="es">(Alfredo Di Stéfano Laulhé; 아르헨티나 출신 센터 포워드, 1926-2014)</span>가 왼쪽 안쪽에서 공을 이어받아 낮게 마무리했다. 영상에서 두드러지는 것은 슈팅의 크기보다 공을 처리하는 간격이다. 공을 받은 뒤 자세를 오래 고쳐 잡지 않고 골키퍼가 몸을 낮추는 사이 슈팅을 끝냈다. 랭스가 두 골 차를 누린 시간은 4분이었다.<a class="cite" href="#source-1" aria-label="출처 1">[1]</a></p>
  <p>30분에는 엑토르 리알<span class="foreign-note" lang="es">(José Héctor Rial Laguía; 아르헨티나·스페인 공격수, 1928-1991)</span>이 동점을 만들었다. 디스테파노의 만회골에서 16분, 랭스의 두 번째 골에서 20분 만이었다. 이 두 골만으로 레알이 랭스의 측면 공격에 해법을 찾았다고 말할 수는 없다. 다만 랭스 역시 자기 골문 앞에서 레알의 마무리를 막지 못하고 있었다. 전반에는 레알의 슈팅이 크로스바를 때리는 장면도 나왔고, 두 팀은 2-2로 하프타임을 맞았다.<a class="cite" href="#source-6" aria-label="출처 6">[6]</a></p>
</section>

<div class="european-cup-transition european-cup-half-time" aria-label="전반 종료, 레알 마드리드 CF 2대2 스타드 드 랭스">
  <div class="transition-kicker"><span>PARIS · 13 JUNE 1956</span><b>THE FIRST EUROPEAN CUP FINAL</b></div>
  <div class="transition-score">
    <div class="transition-team"><img class="match-board-crest" src="/images/archive/teams/clubs/real-madrid-1941.svg" alt="레알 마드리드 CF 문장" width="80" height="88" /><strong>레알 마드리드 CF</strong><small>REAL MADRID CF</small><span class="match-board-country"><img src="/images/flags/es-1945.png" alt="1945년부터 사용된 스페인 국기" width="28" height="18" /><span>스페인</span></span></div>
    <div class="transition-numbers"><span>2</span><em>HALF<br />TIME</em><span>2</span></div>
    <div class="transition-team"><img class="match-board-crest" src="/images/archive/teams/clubs/reims-1931.svg" alt="스타드 드 랭스 문장" width="80" height="88" /><strong>스타드 드 랭스</strong><small>STADE DE REIMS</small><span class="match-board-country"><img src="/images/flags/fr.svg" alt="프랑스 국기" width="28" height="18" /><span>프랑스</span></span></div>
  </div>
  <p>REIMS LED 2-0 · MADRID ANSWERED BEFORE THE BREAK</p>
</div>

## 코파가 상대편 선수가 되기 직전의 결승

전반의 네 골 사이에서 가장 유명한 프랑스 선수의 이름은 득점자 명단에 없었다. 레몽 코파<span class="foreign-note" lang="fr">(Raymond Kopaszewski; 프랑스 공격수, 1931-2017)</span>는 중앙과 측면을 오가며 랭스의 공격을 연결했다. UEFA와 스타드 드 랭스의 회고에 따르면 그는 이미 다음 시즌 레알 이적을 앞둔 상태였다.<a class="cite" href="#source-6" aria-label="출처 6">[6]</a><a class="cite" href="#source-8" aria-label="출처 8">[8]</a>

그러니 이 경기는 흔히 말하는 ‘코파 더비’보다 조금 이상한 시간에 놓여 있었다. 코파는 미래의 동료들을 상대로 랭스의 첫 유럽 우승을 만들어야 했고, 레알은 곧 자기 선수가 될 공격수를 막아야 했다. 오늘날의 이적 발표식처럼 미래의 유니폼이 현재의 충성을 대신해주지는 않았다. 코파가 훗날 랭스의 축구를 ‘샴페인 풋볼’이라고 부른 까닭도 이 팀이 거쳐 가는 발판이어서가 아니라, 자신이 뛰어본 가장 아름다운 공격 축구 가운데 하나였기 때문이다.<a class="cite" href="#source-8" aria-label="출처 8">[8]</a>

## 후반 62분: 이달고가 다시 랭스를 앞세우다

<section class="highlight-scene european-cup-scene">
  <header><span>62' · STADE DE REIMS 3-2</span><h3>측면을 흔들던 선수가 직접 골문에 도착하다</h3></header>
  <div class="highlight-clip-grid"><figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="미셸 이달고의 1956 유러피언컵 결승전 헤더 골"><source src="/media/highlights/1955-56-european-cup-final/07-hidalgo-62.mp4" type="video/mp4" /></video><figcaption>높게 넘어온 공을 이달고가 머리로 돌려놓으며 랭스가 두 번째 리드를 잡았다.</figcaption></figure></div>
  <p>후반 17분, 전반에 측면을 넓혔던 미셸 이달고<span class="foreign-note" lang="fr">(Michel Hidalgo; 프랑스 측면 공격수, 1933-2020)</span>가 이번에는 골문 앞에 도착했다. 높게 넘어온 공을 헤더로 돌려놓으며 랭스가 3-2로 다시 앞섰다. 이달고는 훗날 프랑스 대표팀 감독으로 1984 UEFA 유로 우승을 이끈다. 그러나 1956년의 화면 속 그는 전술을 설명하는 지도자가 아니라, 직접 폭을 만들고 마지막 공간까지 침투한 23세 공격수였다.<a class="cite" href="#source-7" aria-label="출처 7">[7]</a></p>
  <p>이 골은 전반 2-0이 우연한 돌풍이 아니었음을 확인했다. 레알이 두 골을 따라잡고 하프타임을 맞았어도 랭스의 공격 구조는 사라지지 않았다. 두 번째 리드까지 랭스의 세 골은 서로 다른 선수가 넣었고, 이달고는 측면의 원인과 문전의 결과에 모두 관여했다.</p>
</section>

## 후반 67분: 마르키토스가 공격수가 된 순간

<section class="highlight-scene european-cup-scene">
  <header><span>67' · 3-3</span><h3>수비수의 전진이 두 번째 동점을 만들다</h3></header>
  <div class="highlight-clip-grid two-clips">
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="세 번째 실점 뒤 다시 공격하는 레알 마드리드"><source src="/media/highlights/1955-56-european-cup-final/08-real-response.mp4" type="video/mp4" /></video><figcaption>중앙선 부근에서 상대를 등지고 공을 지키는 레알 선수.</figcaption></figure>
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="마르키토스의 1956 유러피언컵 결승전 동점골"><source src="/media/highlights/1955-56-european-cup-final/09-marquitos-67.mp4" type="video/mp4" /></video><figcaption>여러 선수가 엉킨 문전에서 마르키토스가 공격에 가담해 3-3을 만들었다.</figcaption></figure>
  </div>
  <p>랭스의 세 번째 득점 뒤 이어지는 첫 영상에서 레알 선수는 중앙선 부근에서 상대를 등지고 공을 지킨 뒤 몸을 돌린다. 두 번째 영상은 67분의 동점골이다. 마르키토스<span class="foreign-note" lang="es">(Marcos Alonso Imaz; 스페인 수비수, 1933-2012)</span>가 문전까지 올라왔고, 여러 선수가 엉킨 자리에서 골을 넣었다. 공이 어느 발에 어떻게 닿았는지는 짧은 화면에서 선명하지 않지만, 수비수가 공격이 끝나는 위치까지 따라 들어온 것은 보인다. 디스테파노와 리알만 막으면 되는 공격이었다면, 그 자리에 마르키토스가 있을 이유도 없었을 것이다.</p>
  <p>3-3. 레알은 세 번 앞선 팀이 아니라, 두 번 뒤처지고 두 번 따라붙은 팀이었다. 그리고 결승의 첫 78분 동안 한 번도 리드하지 못했다.</p>
</section>

## 후반 79분: 리알의 첫 터치, 레알의 첫 리드

<section class="highlight-scene european-cup-scene">
  <header><span>79' · REAL MADRID CF 4-3</span><h3>엑토르 리알, 결승의 방향을 처음 뒤집다</h3></header>
  <div class="highlight-clip-grid two-clips">
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="엑토르 리알의 1956 유러피언컵 결승전 결승골"><source src="/media/highlights/1955-56-european-cup-final/10-rial-79.mp4" type="video/mp4" /></video><figcaption>리알은 첫 터치로 수비의 접근을 벗어난 뒤 전진해 자신의 두 번째 골을 넣었다.</figcaption></figure>
    <figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="레알 마드리드의 네 번째 골에 환호하는 관중석"><source src="/media/highlights/1955-56-european-cup-final/11-madrid-crowd.mp4" type="video/mp4" /></video><figcaption>결승에서 처음 앞선 레알을 향해 관중석의 흰 깃발과 응원 문구가 흔들린다.</figcaption></figure>
  </div>
  <p>리알은 첫 터치로 공을 자기 진행 방향에 놓고 수비수의 접근을 벗어났다. 이어진 슈팅이 골문을 가르면서 레알은 후반 34분에야 처음 앞섰다. 리알에게는 전반 동점골에 이은 두 번째 득점이었다. 디스테파노와 리알은 대회에서 각각 다섯 골을 넣었고, 결승에서는 세 골을 나눠 가졌다.<a class="cite" href="#source-4" aria-label="출처 4">[4]</a></p>
  <p>오늘의 시선은 이 장면을 왕조의 출발이라고 부르기 쉽다. 그러나 화면 안의 선수들이 알고 있던 것은 훨씬 작고 급한 사실이었다. 남은 시간은 11분, 한 골 차, 그리고 결승에서 처음 얻은 리드. 헨토는 훗날 “우리가 역사를 만들고 있다는 것을 몰랐다”고 말했다.<a class="cite" href="#source-3" aria-label="출처 3">[3]</a> 그들 앞에 있던 것은 지키지 못하면 사라질 한 골 차였다. 역사라는 이름은 나중에 붙었다.</p>
</section>

<div class="european-cup-transition european-cup-full-time" aria-label="경기 종료, 레알 마드리드 CF 4대3 스타드 드 랭스">
  <div class="poster-shade"></div>
  <div class="transition-kicker"><span>FULL TIME · PARC DES PRINCES</span><b>REAL MADRID CF 4-3 STADE DE REIMS</b></div>
  <div class="poster-copy"><small>13 JUNE 1956</small><strong>라 프리메라</strong><p>THE FIRST EUROPEAN CUP · THE FIRST CHAMPION</p></div>
  <div class="poster-winner"><img src="/images/flags/es-1945.png" width="120" height="80" alt="1945년부터 사용된 스페인 국기" /><span>CHAMPION<br /><b>REAL MADRID CF</b></span></div>
</div>

## 첫 결승에 도착한 두 팀, 첫 우승과 첫 준우승

초대 대회의 결승 진출은 두 구단 모두 처음이었다. 레알 마드리드 CF는 첫 우승을, 스타드 드 랭스는 첫 준우승을 기록했다. 아래 횟수는 이후의 성적을 더하지 않은 1955-56시즌 종료 시점의 통산 기록이다.

### 결승 진출

<div data-european-cup-milestone="1955-56:final"></div>

### 우승

<div data-european-cup-milestone="1955-56:champions"></div>

### 준우승

<div data-european-cup-milestone="1955-56:runners-up"></div>

## 트로피를 든 미겔 무뇨스

<section class="highlight-scene european-cup-scene european-cup-trophy-scene">
  <header><span>AFTER FULL TIME · THE CAPTAIN</span><h3>선수로 처음 들고, 감독으로 다시 들 사람</h3></header>
  <div class="highlight-clip-grid"><figure><video class="highlight-clip" data-autoplay-on-view muted loop playsinline controls preload="none" aria-label="미겔 무뇨스가 1956 유러피언컵 트로피를 들어 올리는 장면"><source src="/media/highlights/1955-56-european-cup-final/12-munoz-trophy.mp4" type="video/mp4" /></video><figcaption>주장 미겔 무뇨스가 초대 유러피언컵 트로피를 머리 위로 들어 올린다.</figcaption></figure></div>
  <p>마지막 영상에서 트로피를 드는 사람은 미겔 무뇨스<span class="foreign-note" lang="es">(Miguel Muñoz Mozún; 스페인 미드필더·주장, 1922-1990)</span>다. 레알 마드리드 공식 기록에 따르면 그는 구단 주장으로 유러피언컵을 처음 들어 올렸고, 선수로 세 차례 우승한 뒤 감독으로 두 차례 더 우승했다. 14시즌 연속 레알을 지휘한 구단 최장수 감독이기도 하다.<a class="cite" href="#source-5" aria-label="출처 5">[5]</a></p>
  <p>그러나 1956년의 무뇨스가 들어 올린 것은 아직 왕조의 상징물이 아니었다. 새 대회의 초대 우승컵이었다. 결승에서 두 골을 먼저 내주고, 한 번 더 뒤처지고, 79분에야 처음 앞선 팀이 간신히 얻은 물건이었다.</p>
</section>

‘라 프리메라’<span class="foreign-note" lang="es">(La Primera)</span>는 스페인어로 ‘첫 번째’를 뜻하며, 레알 마드리드의 첫 유러피언컵 우승을 가리키는 표현이다. 실제 경기 안에서 첫 번째는 그렇게 단정한 숫자가 아니었다. 정보가 부족한 낯선 대회, 양쪽에서 수비를 찢은 프랑스 챔피언, 두 골 차와 세 번째 실점, 수비수의 문전 가담과 79분의 첫 리드가 한꺼번에 얽힌 결과였다.

스타드 드 랭스는 레알의 역사를 위해 준비된 조연이 아니었다. 두 번 앞섰고 세 명의 선수가 골을 넣었으며, 전반의 두 골 차를 잃은 뒤에도 다시 리드를 잡았다. 그래서 레알의 첫 우승도 두 차례 동점을 만든 뒤 79분에 비로소 경기를 뒤집어 얻은 승리로 읽어야 한다.

역사는 결과를 안 뒤에는 곧잘 직선이 된다. 1956년의 영상은 그 직선을 다시 구부린다. 레알 마드리드의 유러피언컵 5연패는 첫날부터 보이던 미래가 아니었다. 첫날에는 그저, 79분에 처음 앞선 결승전이 있었다.

<figure class="european-cup-photo"><img src="/images/highlights/1955-56-european-cup-final-kopa-di-stefano.jpg" alt="스타드 드 랭스의 레몽 코파와 레알 마드리드의 알프레도 디스테파노가 악수하는 편집 이미지" loading="lazy" width="966" height="966" /><figcaption>레몽 코파와 알프레도 디스테파노. 코파는 이 결승 뒤 레알 마드리드로 옮겨 1957·1958·1959년 세 차례 유러피언컵 우승을 경험했다.<a class="cite" href="#source-9" aria-label="출처 9">[9]</a></figcaption></figure>

<div class="source-notes">
  <h2>영상과 참고 자료</h2>
  <ol>
    <li id="source-1"><a href="https://www.uefa.com/uefachampionsleague/news/0258-0e6a0359223b-d180c4119f8e-1000--1955-56-madrid-claim-first-crown/">“1955/56: Madrid claim first crown”</a>, UEFA. 결승 득점 순서와 초대 대회 진행.</li>
    <li id="source-2"><a href="https://www.uefa.com/uefachampionsleague/news/021f-0e8ff6a59488-11757c2bbc94-1000--legendary-moments-madrid-s-dynasty-begins/">“Legendary Moments: Madrid's dynasty begins”</a>, UEFA. 파코 헨토가 회고한 첫 대회의 정보 환경.</li>
    <li id="source-3"><a href="https://www.uefa.com/uefachampionsleague/news/01c8-0e6ee14075a4-c14b6cae3094-1000--the-history-makers-of-madrid/">“The history-makers of Madrid”</a>, UEFA. 헨토의 초대 우승 회고.</li>
    <li id="source-4"><a href="https://www.realmadrid.com/landings/copas-de-europa-ganadas/primera-copa-de-europa.html">“La primera Copa de Europa”</a>, Real Madrid CF. 득점·선발 명단과 대회 득실 기록.</li>
    <li id="source-5"><a href="https://www.realmadrid.com/en-US/the-club/history/football-legends/miguel-munoz-mozun">“Miguel Muñoz”</a>, Real Madrid CF. 주장으로서의 첫 우승과 선수·감독 경력.</li>
    <li id="source-6"><a href="https://www.stade-de-reims.com/il-y-a-70-ans-reims-etait-si-pres-du-toit-de-leurope/">“Il y a 70 ans, Reims était si près du toit de l’Europe”</a>, Stade de Reims. 바토의 측면 운용, 득점 장면과 코파의 이적 상황.</li>
    <li id="source-7"><a href="https://www.stade-de-reims.com/disparition-de-michel-hidalgo-le-sdr-en-deuil/">“Disparition de Michel Hidalgo”</a>, Stade de Reims. 이달고의 랭스 경력과 결승 득점.</li>
    <li id="source-8"><a href="https://www.uefa.com/news-media/news/01ee-0e76fea7895f-3c3039e41339-1000--goals-not-coal-for-kopa/">“Goals, not coal, for Kopa”</a>, UEFA. 코파가 회고한 랭스의 공격 축구와 레알 이적.</li>
    <li id="source-9"><a href="https://www.uefa.com/uefachampionsleague/news/0237-0e966287a93d-1e5b23df67db-1000--european-football-mourns-madrid-great-kopa/">“European football mourns Madrid great Kopa”</a>, UEFA. 코파의 1957·1958·1959년 유러피언컵 우승.</li>
    <li><a href="https://www.uefa.com/MultimediaFiles/Download/EuroExperience/uefaorg/Publications/01/53/55/84/1535584_DOWNLOAD.pdf">“UEFA Champions League Statistics Handbook 2011/12”</a>, UEFA. 공식 선발 명단·주심·관중 38,239명 대조.</li>
  </ol>
</div>
