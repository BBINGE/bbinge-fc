...
try {
  for (const width of [380, 768, 1440]) {
    ...
    assert.equal(await page.locator('.cup-goal-row').count(), 7, 'scorer rows: 2 + 3(replay) + 2');
    const goalOverflow = await page.locator('.cup-goal-row p').evaluateAll((els) => els.filter((el) => el.scrollWidth > el.clientWidth + 1).length);
    assert.equal(goalOverflow, 0, 'scorer text fits');
