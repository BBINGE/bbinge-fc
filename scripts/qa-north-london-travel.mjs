...
// 북런던 축행: 지도 핀 22개·필터, 숙소 3곳, 사진 로딩, 영상 클릭 재생, 380/768/1440 가로 넘침.
const base = process.env.QA_BASE || 'http://127.0.0.1:4323';
const out = process.env.QA_OUT || mkdtempSync(join(tmpdir(), 'bbinge-northlondon-'));
const path = '/pilgrimage/north-london-arsenal-tottenham-football-travel/';
...
try {
  for (const width of [380, 768, 1440]) {
    ...
    assert.equal(await page.locator('.journey-live-map__marker').count(), 22, 'twenty-two map pins');
    ...
    assert(src && src.includes('youtube-nocookie.com/embed/a1ei32N_yUI'), 'video plays after click: ' + src);
    ...
    console.log(JSON.stringify({ width, pins: 22, stays: 3, overflow: false }));
    await page.close();
  }
  console.log('NORTH LONDON TRAVEL: PASS');
