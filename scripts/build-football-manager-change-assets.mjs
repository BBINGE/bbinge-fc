...
const [managerSource, benchSource, videoSource, paperFigureSource] = await Promise.all([
  fetchBuffer('https://upload.wikimedia.org/wikipedia/commons/b/b3/Carlo_Ancelotti_Brazil_V_Morocco_13_June_2026-47.jpg'),
...
const base = sharp(managerSource)
  .resize(1920, 1920, { fit: 'cover', position: 'north' })
  .modulate({ saturation: 0.96, brightness: 0.98 })
  .sharpen();

const cover = await base.clone().webp({ quality: 92 }).toBuffer();
await sharp(cover).toFile(path.join(assetDir, 'cover-ancelotti.webp'));
await sharp(cover).resize(1200, 1200).webp({ quality: 90 }).toFile(path.join(assetDir, 'card-ancelotti.webp'));
