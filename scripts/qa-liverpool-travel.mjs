...
// 리버풀 축행: 지도 핀 21개·필터, 숙소 3곳, 사진 로딩, 영상 클릭 재생, 380/768/1440 가로 넘침.
const base = process.env.QA_BASE || 'http://127.0.0.1:4323';
const out = process.env.QA_OUT || mkdtempSync(join(tmpdir(), 'bbinge-liverpool-'));
const path = '/pilgrimage/liverpool-anfield-everton-football-travel/';
...
try {
  for (const width of [380, 768, 1440]) {
    ...
    assert.equal(await page.locator('.journey-live-map__marker').count(), 21, 'twenty-one map pins');
    ...
    assert(src && src.includes('youtube-nocookie.com/embed/l1tneKMd4sE'), 'video plays after click: ' + src);
    ...
    console.log(JSON.stringify({ width, pins: 21, stays: 3, overflow: false }));
    await page.close();
  }
  console.log('LIVERPOOL TRAVEL: PASS');
