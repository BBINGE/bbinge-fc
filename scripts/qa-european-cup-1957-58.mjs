...
// 1957-58 유러피언컵 본선 편 화면 검수: 20개 대진 카드, 재경기 3건(동전 던지기 1건), 누적 표, 모바일 넘침.
const base = process.env.QA_BASE || 'http://127.0.0.1:4321';
const out = process.env.QA_OUTPUT || mkdtempSync(join(tmpdir(), 'bbinge-cup-5758-qa-'));
const path = '/archive/club/european-cup/1957-58-european-cup/';
...
try {
  ...
  for (const width of [380, 768, 1440]) {
    ...
    assert.equal(await page.locator('.cup-tie').count(), 20);
    assert.equal(await page.locator('.cup-leg-results--replay').count(), 3);
    assert.equal(await page.getByText('동전 던지기로 결정', { exact: true }).count(), 1);
    assert.equal(await page.locator('.cup-participant-scroll tbody tr').count(), 24);
    assert.equal(await page.getByText('통산 두 번째 유러피언컵 출전', { exact: true }).count(), 8);
    ...
    assert(data.figures.every(Boolean) && data.figures.length === 4, `figures at ${width}: ${data.figures}`);
    assert.equal(data.flags, 40);
    assert.equal(data.slots, 40);
    assert.equal(data.crests, 40);
    assert(data.uniformSlots);
    assert.equal(data.replayOverlap, false);
    assert.equal(data.headings, 20);
    assert.equal(data.smallText, 0);
    for (const number of [1, 3, 4, 5, 7, 8, 9, 12, 15, 17, 18, 19]) {
  ...
  assert.equal(await plain.locator('.cup-tie').count(), 20);
