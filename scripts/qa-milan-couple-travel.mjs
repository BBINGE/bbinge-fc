...
// 밀라노 커플 축행: 지도 핀 23개·필터, 숙소 3곳, 사진 로딩, 영상 클릭 재생, 380/768/1440 가로 넘침.
...
try {
  for (const width of [380, 768, 1440]) {
    ...
    assert.equal(await page.locator('.journey-live-map__marker').count(), 23, 'twenty-three map pins');
    ...
    console.log(JSON.stringify({ width, pins: 23, stays: 3, overflow: false }));
    await page.close();
  }
