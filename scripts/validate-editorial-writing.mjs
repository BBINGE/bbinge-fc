...
const canonicalTermRules = [
  ...
  [/젱겔러 줄러/g, '젱겔레르 줄러'],
  [/리뉘스 미헬스/g, '리뉘스 미헐스'],
  [/리누스 미헐스|리누스 미헬스/g, '리뉘스 미헐스'],
  [/조제 모리뉴|주제 무리뉴|조제 무리뉴/g, '주제 모리뉴'],
  [/알베르토 스키아피노/g, '후안 스키아피노'],
  [/프란시스코 헨토/g, '파코 헨토'],
  [/마르코스 알론소 이마스|알론소 이마스/g, '마르키토스'],
  [/페렌츠 푸슈카시/g, '푸슈카시 페렌츠'],
  [/매직 머저르/g, '매직 마자르'],
  [/어러니처퍼트/g, '아라니처파트'],
  [/캄프 ?누|캄프 ?노우|캄노우/g, '캄 노우'],
  // 포르투갈어 어말 o는 ㅜ로 적는다. 한국어 위키백과 표제어도 데쿠다(운영자 지적, 2026-09-20).
  // 건축 양식 아르데코는 걸리지 않게 앞의 '아르'를 제외한다.
  [/(?<!아르)데코/g, '데쿠'],
  // 스페인어·카탈루냐어 ll은 모음 앞에서 ㅇ계로 적는다(세비야·마요르카와 같은 규칙).
  // 한국어 위키백과 표제어도 안토니 라마예츠다(운영자 지적, 2026-09-20).
  [/라말레츠/g, '라마예츠'],
  // 프랑스어 Thuram의 어말 m은 ㅁ이다. 한국어 위키백과 표제어도 릴리앙 튀람·마르퀴스 튀람이며
  // 케프렌 튀람까지 삼부자가 모두 튀람이다(운영자 확정, 2026-09-20).
  [/튀랑/g, '튀람'],
  // 아르투르 프리덴라이히. 전용 아카이브 편이 쓰는 표기로 맞춘다(2026-09-20).
  [/프리덴라이시/g, '프리덴라이히'],
  // Louis van Gaal의 Louis는 어말 s를 읽지 않는다. 한국어 위키백과 표제어도 루이 판 할이다.
  [/루이스 판 ?할/g, '루이 판할'],
  // 한국어 위키백과 표제어는 유프 하인케스다. 엠게 편과 축쿼드 블록도 하인케스를 쓴다.
  [/하잉케스/g, '하인케스'],
  // 스웨덴 Ove Kindvall. 1970년 유러피언컵 결승 결승골의 주인공이다.
  [/신드발/g, '킨드발'],
  // 스페인어 González는 곤살레스다. 저장소의 다른 여덟 곳도 곤살레스를 쓴다.
  [/곤잘레스/g, '곤살레스'],
  // 네덜란드어 인명의 전치사 van은 성에 붙여 쓴다(운영자 확정, 2026-09-20).
  // 지명 `콥 판 자위트`(Kop van Zuid)처럼 사람 이름이 아닌 것은 띄운 채로 둔다.
  [/판 데르 사르/g, '판데르사르'],
  [/판 바스턴/g, '판바스턴'],
  [/판 브뢰켈런/g, '판브뢰켈런'],
  [/판 아를러/g, '판아를러'],
  ...
  [/판 페르시/g, '판페르시'],
  [/판 니스텔로이/g, '판니스텔로이'],
  // `판`이 한 낱말일 때만 본다. 막판·전광판·재판·출판처럼 뒤에 붙은 판은 걸리지 않는다.
  [/(?<=^|[\s>(”‘'"])판 할/gm, '판할'],
  // XI는 공개 한국어 문맥에서 베스트 11로 쓴다(선수 규칙 §4). 운영자가 디자인한 표제 이미지에
  // 실제로 XI가 박혀 있는 경우의 대체문구는 보이는 것을 적어야 하므로 alt 안은 검사하지 않는다.
  [/베스트 ?XI(?![^<]*"\s*(loading|width|height))/g, '베스트 11'],
  [/사비 알론소/g, '샤비 알론소'],
  [/사비 에르난데스/g, '차비 에르난데스'],
  // 원어 병기 안의 생몰년은 한 형식으로 적는다(편집·정정 원칙 3항 표현 교정, 2026-09-16). 생존은 1968-, 사망은 1931-2017.
  [/(?<=class="foreign-note"[^<]{0,200})\d{4}년생/g, '1968- 형식'],
  [/(?<=class="foreign-note"[^<]{0,200})\d{4}~\d{4}/g, '1931-2017 형식'],
];
...
const formulaPatterns = [
  [/(?:단순히|그냥|그저)[^.\r\n]{0,45}(?:부르면|설명하면|읽으면|보면|기억하면)[^.\r\n]{0,25}(?:절반|놓친다|못 한다|못한다)/g,
    "'…라고만 부르면 절반만 본다·놓친다' 상투구입니다. 그 글의 내용으로 다시 쓰십시오."],
  [/단순히[^.\r\n]{0,40}(?:이|가) 아니라/g,
...
function runSelfTest() {
  const fixtures = [
    ...
    ['부르면 놓친다 변형', '이것을 단순히 ‘영국식 변형’이라고 부르면 변화의 주체를 놓친다.', 'PROSE-FORMULA'],
